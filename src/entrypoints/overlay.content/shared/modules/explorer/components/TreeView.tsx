...
export const TreeView = forwardRef<ArboristTreeHandle, TreeViewProps>(({ onSelect }, ref) => {
  const {
    treeRef,
    containerRef,
    dimensions,
  ...
  const data = useMemo(() => {
    ...
    const ignoredSet = new Set(ignoredFolders.map(f => f.toLowerCase().trim()));
    ...
    // Filter by favorites
    if (onlyFavorites) {
        const favoriteIds = new Set(favorites.filter(f => f.target_type === 'conversation').map(f => f.target_id));
    ...
    if (tagFilter.selected.length > 0) {
        ...
        filteredConversations = filteredConversations.filter(c => matchingConversationIds.has(c.id));
    }
